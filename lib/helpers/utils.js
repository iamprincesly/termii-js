 ...
 * Throw TermiiError if recipients is not string or array
 *
 * @param {string | string[]} recipients
 * @return {string | string[]} recipients
 */
function validateRecipients(recipients) {
    if (!isStringOrArray(recipients)) {
        throw new TermiiError(
            'Phone numbers must be strings or arrays of numbers'
        );
    }

    // Please note: the array can take up to 10,000 phone numbers
    if (isArray(recipients) && recipients.length > 10000) {
        throw new TermiiError(
            'You can only send bulk SMS to 10000 phone numbers at a time.'
        );
    }
 ...
 * Throw TermiiError if not a string
 * and return message if is a string
 *
